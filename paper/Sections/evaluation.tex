...
\begin{itemize}[leftmargin=*]
  \item \textbf{RQ1 (Effectiveness).} Does providing code context as images affect patch success on SWE-bench Verified compared to plain-text context?
  \item \textbf{RQ2 (Efficiency).} How does optical encoding affect token usage, dollar cost, and wall clock time?
  \item \textbf{RQ3 (Fidelity).} How accurately can GPT-5-mini read Python source code from rendered images, and which code features are most error-prone?
\end{itemize}
...
We apply each generated patch and run the benchmark's containerized test suite, following the official SWE-bench protocol: \texttt{FAIL\_TO\_PASS} tests must pass, and \texttt{PASS\_TO\_PASS} tests must remain passing \cite{jimenez2023swebench}.

\para{Model and agent.}
We use GPT-5-mini (\texttt{gpt-5-mini-2025-08-07}) as the backbone model within the \texttt{mini-swe-agent} framework. The agent has a 50-step limit and a \$3 cost limit per instance. All other settings (prompt template, tool interface, environment) are identical across conditions.

\para{Conditions.}
We compare two conditions:
\begin{itemize}[leftmargin=*]
  \item \textbf{Text.} Standard setup---all tool outputs are returned as plain text.
...
\end{itemize}
Both conditions use identical prompts except that the optical prompt includes one additional system instruction (``Some code content may be provided as images'') and an explicit ``do not \texttt{git commit}'' directive (see Section~\ref{sec:prompt-alignment}).
...
In an initial 20-instance pilot, we observed that image inputs caused GPT-5-mini to alter its workflow behavior: the model ran \texttt{git commit} before generating the patch in 85--90\% of optical runs versus only 10\% of text runs, causing \texttt{git diff} to return empty output. Table~\ref{tab:prompt-alignment} shows this effect and its resolution.
...
After adding the instruction ``Do NOT run \texttt{git add} or \texttt{git commit}; your changes must remain as uncommitted modifications,'' the commit behavior dropped to 0\% and the empty-patch rate matched the text baseline. All main experiments use the fixed prompt. This finding is noteworthy in its own right: multimodal inputs can alter model behavior in ways entirely unrelated to the content, a confound that must be controlled in any fair comparison.


% ===================================================================
\subsection{Image Readability Pilot}\label{sec:rendering-config}
% ===================================================================
...
\para{Motivation.}
If the model cannot reliably recover code from images, any downstream performance difference would be uninterpretable---we would not know whether failures stem from optical compression itself or from poor rendering choices. This section establishes a lower bound on image readability before proceeding to the main evaluation.
...
\emph{Header injection.} When content spans multiple images, the model occasionally inserts the header from the next image (e.g., ``\texttt{filename.py (part 2/2)}'') into the transcription output. This is a predictable, strippable artifact.
...
% ===================================================================
\subsection{Main Results}\label{sec:eval-results}
% ===================================================================
...
We also note that GPT-5-mini does not support \texttt{temperature=0}, so all outputs involve sampling noise. Some of the 20 disagreement instances may simply reflect run-to-run variance rather than genuine modality effects. Disentangling these factors would require multiple runs per instance per condition, which we leave to future work.
...
\para{RQ3: Fidelity and code detection.}
