...
\para{Input \& Output.}
We study a long-context coding task in an agentic setting: given (i)~a natural-language issue description and (ii)~access to a software repository at a fixed snapshot, the model interacts with a shell environment over multiple turns---reading files, running tests, and editing code---to produce a patch that resolves the described issue \cite{jimenez2023swebench}. We build on \texttt{mini-swe-agent} \cite{yang2024sweagent}, a minimal agentic framework where the agent repeatedly queries a language model, executes bash commands, and observes the output.

\para{Main Idea.}
Our core idea is to replace the \emph{text} representation of code with an \emph{image} representation wherever the agent encounters source code during its workflow. Concretely, when a tool command returns a code-heavy output (e.g., reading a file with \texttt{sed~-n} or \texttt{cat}), we render that output as a monospace document image and present it to the vision-capable model as an image input instead of text. This lets us test whether a model can perform multi-step software engineering through the visual channel without any post-training or architectural change.

The key insight is that code reading happens predominantly during the agent loop, not in the initial problem description. Our analysis of agent traces shows that \texttt{sed}/\texttt{cat}-style file reads account for over 80\% of the code tokens the model processes, while code blocks in the issue description are a small fraction. Therefore, we apply optical encoding at the \emph{observation level}---intercepting tool outputs---rather than only at the input level.
...
A technical constraint shapes the implementation: the OpenAI tool-call API requires tool-response messages to contain only text, not images. We work around this by (1)~replacing the code output in the tool message with a short placeholder (e.g., ``\texttt{[Code output (243 lines) rendered as image in the next message]}'') and (2)~injecting a follow-up \texttt{user} message containing the rendered images. The model thus sees the tool acknowledgment followed immediately by the visual code content.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Code ratio $>$ 20\%}, where ``code ratio'' is the fraction of lines whose stripped content starts with a Python keyword or indentation pattern (\texttt{def}, \texttt{class}, \texttt{import}, \texttt{from}, \texttt{if}, \texttt{for}, \texttt{return}, \texttt{while}, \texttt{try:}, \texttt{\#}, or four-space indent).
\end{itemize}
...
\midrule
\texttt{sed -n '1,240p' models.py} & 7{,}859 & 31\% & Image \\
\texttt{cat utils.py} & 7{,}574 & 33\% & Image \\
\texttt{python3 repro.py} & 1{,}052 & 35\% & Text \\
\texttt{grep -n "def prepare"} & 87 & 0\% & Text \\
\texttt{ls -la} & 1{,}030 & 0\% & Text \\
\texttt{git diff -{}- file.py} & 1{,}268 & 6\% & Text \\
\bottomrule
\end{tabular}
\end{table}
...
\uIf{$|t| > 2000$ \textbf{and} $r > 0.2$}{
  ...
  $m_{\mathrm{tool}} \leftarrow \texttt{"[Code output (}n\texttt{ lines) rendered as image]"}$\;
...
\para{Rendering example.}
Figure~\ref{fig:rendering-example} shows a representative rendered image produced by our pipeline. The image uses 12pt DejaVu Sans Mono on a white background, with a gray header bar indicating the source file. This is the exact format the model receives when a code-reading command (e.g., \texttt{sed~-n~'1,240p'~fields/\_\_init\_\_.py}) returns a code-heavy output.
...
We implement the optical encoding as a subclass of the default agent (\texttt{OpticalAgent}) that overrides the \texttt{execute\_actions} method. The agent loop proceeds as follows: (1)~the model receives the conversation history and generates a bash command via tool call; (2)~the environment executes the command and returns the output; (3)~the \texttt{OpticalAgent} checks whether the output passes the code detection heuristic; (4a)~if yes, the output is rendered as images, the tool message is replaced with a placeholder, and a user message with the images is appended; (4b)~if no, the output is passed as text normally; (5)~the model sees the updated history and generates the next command. This cycle repeats until the model submits a patch or hits the step/cost limit.
